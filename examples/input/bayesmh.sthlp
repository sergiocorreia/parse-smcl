...
{p 16 18 2}
{it:{help bayesmh##modelspec:model}}
...
{synopt :{opt binlogit(n)}}binomial regression with logit link{p_end}
...
See {mansection BAYES bayesmhRemarksandexamplesSpecifyingargumentsoflikelihoodmodelsandpriordistributions:{it:Specifying arguments of likelihood models and prior distributions}}.
...
{synopt :{opt noglmtr:ansform}}do not apply GLM-type transformation to the
linear predictor or nonlinear specification; allowed only with
{cmd:exponential}, {cmd:poisson}, or {cmd:binlogit()}{p_end}
{synoptline}
{p2colreset}{...}
...
and nonlinear models{p_end}
{p2coldent:* {opt likel:ihood}{cmd:(}{it:{help bayesmh##modelspec:modelspec}}{cmd:)}}distribution for the likelihood model{p_end}
...
{syntab :Model 2}
{synopt :{opt redef:ine}{cmd:(}{it:label}{cmd::i.}{it:{help varname:varname}}{cmd:)}}specify a random-effects linear form{p_end}
...
{p 4 6 2}Options {cmd:prior()} and {cmd:block()} can be repeated.{p_end}
...
{pstd}
{cmd:bayesmh} fits a variety of Bayesian models using Metropolis-Hastings (MH)
algorithm.  It provides various likelihood models and prior distributions for
you to choose from.  Likelihood models include univariate normal linear and
nonlinear regressions, multivariate normal linear and nonlinear regressions,
generalized linear models such as logit and Poisson regressions,
and multiple-equations linear models.  Prior distributions include continuous
distributions such as uniform, Jeffreys, normal, gamma, multivariate normal,
and Wishart and discrete distributions such as Bernoulli and Poisson.  You can
also program your own Bayesian models; see
...
{phang}
{marker dist_spec}
{opt likelihood}{cmd:(}{it:{help bayesmh##modelspec:modelspec}}{cmd:)}
specifies the distribution of the data in the regression model.  This option
specifies the likelihood portion of the Bayesian model. This option is
required. {it:modelspec} specifies one of the supported likelihood
distributions.  A location parameter of these distributions is automatically
parameterized as a linear combination of the specified independent variables
and needs not be specified.  Other parameters may be specified as arguments to
the distribution separated by commas. Each argument may be a real number
({it:#}), a variable name (except for matrix parameters), a predefined matrix,
a model parameter specified in {cmd:{c -(} {c )-}}, a Stata expression, or a
substitutable expression containing model parameters; see
...
{pmore}
For some likelihood models, option {cmd:likelihood()} provides suboptions
{it:subopts} in {opt likelihood(..., subopts)}. {it:subopts} is
{cmd:offset()}, {cmd:exposure()}, and {cmd:baseoutcome()}.
...
is available with {cmd:probit}, {cmd:logit}, {cmd:binlogit()}, {cmd:oprobit},
{cmd:ologit}, and {cmd:poisson}.
...
{phang2}
{cmd:noglmtransform} requests that the GLM-type transformation not be applied
to linear and nonlinear predictors of Poisson, exponential, and binomial
logistic regression models.  If this option is not specified, Poisson,
exponential, and binomial logistic regressions are assumed, which apply the
corresponding GLM transformation to the linear predictor to model the mean
function.  The GLM transformation is mu = E(Y|X) = h(X'beta), where h() is an
inverse link function.  For example, for Poisson and exponential regression
models, h(x) = exp(x).  For generalized nonlinear models,
mu = h{c -(}f(X,beta){c )-}, where f() is a nonlinear function of predictors.
If this option is specified, the linear and nonlinear specifications are
assumed to directly model the mean mu of the Poisson distribution, the scale
parameter b=1/mu of the exponential distribution, and the probability of
success mu/n of the binomial distribution.  This option is useful with
constant-only models for modeling outcome distributions directly; see
{mansection BAYES bayesmhRemarksandexamplesBetabinomialmodel:{it:Beta-binomial model}}
under {it:Remarks and examples} in {bf:[BAYES] bayesmh}.
...
models.  See {mansection BAYES bayesmhRemarksandexamplesex29:example 29}.
...
when checking convergence of MCMC.
...
not appear in the estimation table, and postestimation features will not be
available for these parameters.  This option is useful for suppressing
nuisance model parameters.  For example, if you have a factor predictor
variable with many levels but you are only interested in the variability of
the coefficients associated with its levels, not their actual values, then you
may wish to exclude this factor variable from the simulation results.  If you
simply want to omit some model parameters from the output, see the
{cmd:noshow()} option.
...
20,000{p_end}
{phang2}{cmd:. set seed 14}{p_end}
{phang2}{cmd:. bayesmh count = ({mu1}*sign(year<{cp})+{mu2}*sign(year>={cp})),}
	{cmd:likelihood(poisson, noglmtransform)}
